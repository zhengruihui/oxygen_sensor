...
/***************----- 端口数据缓冲区 -----***************/
unsigned char Rx_Buffer[50];	//端口接收数据缓冲区 
unsigned char Tx_Buffer[50];	//端口发送数据缓冲区 
...
void Socket_Init(SOCKET s)
{
	...
	Write_W5500_SOCK_2Byte(0, Sn_MSSR, 60);//最大分片字节数=30(0x001e)
	//设置指定端口
	switch(s)
	{
		case 0:
...
void Load_Net_Parameters(void)
{
	...
	UDP_DPORT[0] = 0x15;//加载端口0的目的端口号6000
	UDP_DPORT[1] = 0x83;
	...
	//Write_SOCK_Data_Buffer(s, "hello\n", size);
        switch(Rx_Buffer[8])
        {
          case 'D':
            ...
            break;
          case 'S':
            if((size - 8) != 10 || Rx_Buffer[8+5] != ',')  //如果字符串格式不是  Sxxxx,xxxx
              break;
            else
            {
